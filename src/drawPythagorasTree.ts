export default function Draw(ctx: CanvasRenderingContext2D, canvasSize: number) {
  ...
  function getLeftPoints(x: number, y: number, size: number, degree: number) {
    return [
      {x: x, y: y},
      {
  ...
  function getRightPoints(x: number, y: number, size: number, degree: number) {
    return [
      {x: x, y: y},
      {
        x: x + Math.cos((degree + baseDegree) * Math.PI / 180) * size,
  ...
  function recDraw(p1: Coord, p2: Coord, size: number, degree: number, n: number, i: number) {
    if (n === 0) {
      return;
    }
    (() => { // 左側
